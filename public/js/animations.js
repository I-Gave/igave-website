...
jQuery(document).ready(function($) {
    ...
    if (isMobile.any === false) {
        ...
        $('#promo .product-holder').css('opacity', 0).each(function(i, el){
        ...
        $('#about .figure-right').css('opacity', 0).each(function(i, el){
            $(el).one('inview', function(isInView) {
                if (isInView) {$(this).addClass('animated fadeInLeft delayp2');}
            });
        });

        $('#about .figure-left').css('opacity', 0).each(function(i, el){
            $(el).one('inview', function(isInView) {
                if (isInView) {$(this).addClass('animated fadeInLeft delayp2');}
            });
        });
        ...
        $('#timeline .timeline-item').css('opacity', 0).each(function(i, el){
            $(el).one('inview', function(isInView) {
                if (isInView) {$(this).addClass('animated fadeInLeft delayp2');}
            });
        });
        ...
        $('#team .profile-holder').css('opacity', 0).each(function(i, el){
            $(el).one('inview', function(isInView) {
                if (isInView) {$(this).addClass('animated fadeInLeft delayp2');}
            });
        });
